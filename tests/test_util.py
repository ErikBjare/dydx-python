...
class TestUtil:
    ...
    def test_util_hash_string(self):
        assert (
            utils.hash_string("baconfries")
            == "0xae5dd6cd2427c8b9f8600be5fe223f87913bb47c1b1cef18792a34033f6d752a"
        )  # noqa: E501
    ...
    def test_sign_order(self):
        signature = utils.sign_order(order=ORDER, private_key=PRIVATE_KEY_1)
        assert signature == ORDER_SIGNATURE

    def test_sign_cancel_order(self):
        signature = utils.sign_cancel_order(
            order_hash=ORDER_HASH, private_key=PRIVATE_KEY_1
        )
        assert signature == CANCEL_ORDER_SIGNATURE

    def test_sign_hash(self):
        signature = utils.sign_hash(hash=ORDER_HASH, private_key=PRIVATE_KEY_1)
        assert signature == ORDER_SIGNATURE
        signature = utils.sign_hash(hash=CANCEL_ORDER_HASH, private_key=PRIVATE_KEY_1)
        assert signature == CANCEL_ORDER_SIGNATURE

    def test_token_to_wei(self):
        assert utils.token_to_wei(11, consts.MARKET_WETH) == 11 * (10 ** 18)
        assert utils.token_to_wei(22, consts.MARKET_DAI) == 22 * (10 ** 18)
        assert utils.token_to_wei(33, consts.MARKET_USDC) == 33 * (10 ** 6)
