...
class TestEth:
    ...
    def test_deposit_dai_success(self):
        client = Client(PRIVATE_KEY_1, node=LOCAL_NODE)
        tx_hash = client.eth.deposit(market=consts.MARKET_DAI, wei=1000)
        self._validate_tx_hash(client, tx_hash)
    ...
    def test_withdraw_eth_success(self):
        client = Client(PRIVATE_KEY_1, node=LOCAL_NODE)
        tx_hash = client.eth.withdraw(market=consts.MARKET_WETH, wei=1000)
        self._validate_tx_hash(client, tx_hash)
    ...
    def test_withdraw_eth_to_success(self):
        client = Client(PRIVATE_KEY_1, node=LOCAL_NODE)
        tx_hash = client.eth.withdraw(market=consts.MARKET_WETH, wei=1000, to=ADDRESS_2)
        self._validate_tx_hash(client, tx_hash)

    def test_withdraw_dai_to_success(self):
        client = Client(PRIVATE_KEY_1, node=LOCAL_NODE)
        tx_hash = client.eth.withdraw(market=consts.MARKET_DAI, wei=1000, to=ADDRESS_2)
        self._validate_tx_hash(client, tx_hash)

    def test_withdraw_unknown_failure(self):
        client = Client(PRIVATE_KEY_1, node=LOCAL_NODE)
        with pytest.raises(ValueError) as error:
            client.eth.withdraw(market=consts.MARKET_INVALID, wei=1000)
        assert "Invalid market number" in str(error.value)
    ...
    def test_withdraw_to_zero_eth_success(self):
        client = Client(PRIVATE_KEY_1, node=LOCAL_NODE)
        tx_hash = client.eth.withdraw_to_zero(market=consts.MARKET_WETH)
        self._validate_tx_hash(client, tx_hash)
    ...
    def test_withdraw_to_zero_eth_to_success(self):
        client = Client(PRIVATE_KEY_1, node=LOCAL_NODE)
        tx_hash = client.eth.withdraw_to_zero(market=consts.MARKET_WETH, to=ADDRESS_2)
        self._validate_tx_hash(client, tx_hash)

    def test_withdraw_to_zero_dai_to_success(self):
        client = Client(PRIVATE_KEY_1, node=LOCAL_NODE)
        tx_hash = client.eth.withdraw_to_zero(market=consts.MARKET_DAI, to=ADDRESS_2)
        self._validate_tx_hash(client, tx_hash)
    ...
    def test_get_wallet_balance(self):
        client = Client(PRIVATE_KEY_1, node=LOCAL_NODE)
        with pytest.raises(web3.exceptions.BadFunctionCallOutput) as error:
            client.eth.get_wallet_balance(address=ADDRESS_2, market=consts.MARKET_DAI)
        assert CONTRACT_NOT_FOUND_ERROR in str(error.value)

    def test_get_wallet_balance_eth(self):
        client = Client(PRIVATE_KEY_1, node=LOCAL_NODE)
        balance = client.eth.get_wallet_balance(
            address=ADDRESS_2, market=consts.MARKET_ETH
        )
        assert balance > 0
    ...
    def test_get_collateralization(self):
        ...
        with pytest.raises(web3.exceptions.BadFunctionCallOutput) as error:
            client.eth.get_collateralization(
                address=ADDRESS_2, accountNumber=1111,
            )
        assert CONTRACT_NOT_FOUND_ERROR in str(error.value)
