...
class Eth(object):
    def __init__(self, node, private_key, public_address, account_number):
        ...
        self.solo_margin = self._create_contract(
            consts.SOLO_MARGIN_ADDRESS, "abi/solomargin.json"
        )
        self.payable_proxy = self._create_contract(
            consts.PAYABLE_PROXY_ADDRESS, "abi/payableproxy.json"
        )
        self.weth_contract = self._create_contract(
            consts.WETH_ADDRESS, "abi/erc20.json"
        )
        self.sai_contract = self._create_contract(consts.SAI_ADDRESS, "abi/erc20.json")
        self.usdc_contract = self._create_contract(
            consts.USDC_ADDRESS, "abi/erc20.json"
        )
        self.dai_contract = self._create_contract(consts.DAI_ADDRESS, "abi/erc20.json")

    # -----------------------------------------------------------
    # Private Helper Functions
    # -----------------------------------------------------------

    def _send_eth_transaction(self, method, options=None):
        if options is None:
            options = dict()
        ...
        if "gas" not in options:
            try:
                options["gas"] = int(
                    method.estimateGas(options) * consts.DEFAULT_GAS_MULTIPLIER
                )
            except Exception:
    ...
    def _operate(self, actionType, market, wei, ref, otherAddress):
        ...
        accounts = [{"owner": self.public_address, "number": self.account_number}]
        amountField = {
            "sign": isDeposit,
            "denomination": 0,  # wei
            "ref": ref,
        ...
        txOptions = dict(
            value=(wei if (isDeposit and market == consts.MARKET_WETH) else 0)
        )

        if market == consts.MARKET_WETH:
            return self._send_eth_transaction(
                self.payable_proxy.functions.operate(
                    accounts, operations, otherAddress
                ),
                options=txOptions,
            )
        else:
            return self._send_eth_transaction(
                self.solo_margin.functions.operate(accounts, operations),
                options=txOptions,
            )

    def _create_contract(self, address, file_path):
    ...
    def _get_token_contract(self, market):
        if market == consts.MARKET_WETH:
    ...
    def set_allowance(self, market):
        ...
        try:
            allowance = contract.functions.allowance(
                self.public_address, consts.SOLO_MARGIN_ADDRESS
            ).call()
            if allowance != 0:
                return
        except Exception:
    ...
    def withdraw(self, market, wei, to=None):
        """
    ...
    def withdraw_to_zero(self, market, to=None):
        """
    ...
    def get_oracle_price(self, market):
        """
    ...
    def get_my_wallet_balance(self, market):
        """
        ...
        :returns: number
        """
        return self.get_wallet_balance(address=self.public_address, market=market)

    def get_wallet_balance(self, address, market):
        """
    ...
    def get_my_collateralization(self):
        """
        Gets collateralization of my account.

        :returns: number
        """
        return self.get_collateralization(self.public_address, self.account_number)

    def get_collateralization(self, address, accountNumber):
        """
    ...
    def get_my_balances(self):
        ...
        :returns: number
        """
        return self.get_balances(self.public_address, self.account_number)

    def get_balances(self, address, accountNumber):
        """
        ...
        result = list(map(lambda wei: wei[1] if wei[0] else -wei[1], weis))
        return result
